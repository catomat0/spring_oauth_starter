...
public class OahSignupTokenProvider {
    ...
    public OahSignupTokenProvider(OahSignupTokenProperties properties) {
    ...
    private static SecretKey buildSigningKey(String secretKey) {
        if (secretKey == null || secretKey.isBlank()) {
            throw new OahSignupTokenException(OahSignupTokenErrorCode.SECRET_KEY_MISSING,
                    "signup-token.secret-key must be configured");
        }
        byte[] keyBytes = secretKey.getBytes(StandardCharsets.UTF_8);
        if (keyBytes.length < MIN_SECRET_BYTES) {
            throw new OahSignupTokenException(OahSignupTokenErrorCode.SECRET_KEY_TOO_SHORT,
    ...
    public OahSignupTokenBuilder builder(String provider, String providerId, String email) {
        return new OahSignupTokenBuilder(this, provider, providerId, email);
    }
    ...
    public OahSignupTokenPayload parse(String token) {
        ...
        if (!TYPE_VALUE.equals(type)) {
            throw new OahSignupTokenException(OahSignupTokenErrorCode.TOKEN_TYPE_MISMATCH,
        ...
        return new OahSignupTokenPayload(provider, providerId, email, extras);
    }
}
