...
public class OahJwtProvider {
    ...
    private final OahJwtProperties properties;
    private final SecretKey signingKey;

    public OahJwtProvider(OahJwtProperties properties) {
    ...
    private static SecretKey buildSigningKey(String secretKey) {
        if (secretKey == null || secretKey.isBlank()) {
            throw new OahJwtException(OahJwtErrorCode.SECRET_KEY_MISSING, "jwt.secret-key must be configured");
        }
        byte[] keyBytes = secretKey.getBytes(StandardCharsets.UTF_8);
        if (keyBytes.length < MIN_SECRET_BYTES) {
            throw new OahJwtException(OahJwtErrorCode.SECRET_KEY_TOO_SHORT,
    ...
    public OahJwtPayload parseAccess(String token) {
        return doParse(token, TYPE_ACCESS);
    }

    public OahJwtPayload parseRefresh(String token) {
        return doParse(token, TYPE_REFRESH);
    }

    private OahJwtPayload doParse(String token, String expectedType) {
        ...
        return new OahJwtPayload(userId, role, type, extras);
    }
}
